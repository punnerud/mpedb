...
| `commit` | pwrite + `msync(MS_SYNC)` before flip (folded into the ONE data span on Linux since #111; per-run on Darwin, where msync costs range width) | after meta msync | Extent ranges are explicitly range-synced in step 3 (§4) — on Linux the no-op `sync_barrier` means these msyncs ARE the ordering, so the extent-dirty list is load-bearing, not an optimization. Double-buffered meta unchanged. |
