...
- **Measured**: burning the whole 4096-id space through real DROP+CREATE commits
  (`drop_table::create_refuses_after_the_lifetime_id_ceiling`) takes **~22 s** run
  alone (~110 s when it shares a loaded 4-core box with the rest of that file) —
  ~5 ms per DDL pair averaged over a record growing from 0 to 4096 tombstones. That
