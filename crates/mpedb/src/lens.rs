//! Lens pairs — reversible PySpell ETL, stage 1 (design/DESIGN-RRETL.md).
//!
...
/// by the engine — never inferred (DESIGN-RRETL §3, commitment 2). Cambria died
...
/// runs against the same inputs (DESIGN-RRETL §5.1).
...
/// `branch_policy` 0: the residual carries the branch tag (DESIGN-RRETL §6).
...
/// and M3 databases (DESIGN-RRETL §8.1).
...
pub struct LensInfo {
    ...
    /// "verified": the evidence is statistical, not universal (DESIGN-RRETL §5).
    pub samples: u32,
...
/// The v1 probe corpus (DESIGN-RRETL §5.1) — the edges that break naive
...
/// added** (DESIGN-RRETL §4): every practical system in the literature dropped it,
...
/// What `rretl apply`/`rretl revert` need from a registered pair (crate::etl).
pub(crate) struct RretlLens {
    pub class: LensClass,
...
impl crate::Database {
    ...
    pub(crate) fn load_lens_for_rretl(&self, name: &str) -> Result<RretlLens> {
        let key = crate::sys_record_subkey(NS_LENS, name.as_bytes())?;
        let r = self.engine.begin_read()?;
        let out = (|| -> Result<RretlLens> {
            let rec = r
                .sys_get(&key)?
                .as_deref()
                .and_then(decode_lens_record)
