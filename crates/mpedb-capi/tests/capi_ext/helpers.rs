...
pub(super) fn scratch_dir() -> String {
...
pub(super) unsafe fn open_memory() -> *mut Sqlite3 {
...
pub(super) unsafe fn exec(db: *mut Sqlite3, sql: &str) -> c_int {
    let s = cs(sql);
...
pub(super) fn fnptr1(f: unsafe extern "C" fn(*mut c_void)) -> *mut c_void {
    f as *const () as *mut c_void
}
...
pub(super) fn sqlite_transient() -> *mut c_void {
    SQLITE_TRANSIENT as *mut c_void
}

pub(super) unsafe fn col_name(st: *mut Stmt, i: c_int) -> String {
...
pub(super) unsafe fn col_text(st: *mut Stmt, i: c_int) -> String {
    let p = sqlite3_column_text(st, i);
    assert!(!p.is_null());
    CStr::from_ptr(p as *const c_char).to_str().unwrap().to_string()
}

pub(super) unsafe fn scalar_count(db: *mut Sqlite3, sql: &str) -> i64 {
    let s = cs(sql);
...
pub(super) unsafe fn collect_text_col(db: *mut Sqlite3, sql: &str) -> Vec<String> {
    let s = cs(sql);
    let mut st: *mut Stmt = ptr::null_mut();
...
pub(super) unsafe extern "C" fn count_destroy(p: *mut c_void) {
...
pub(super) unsafe extern "C" fn reverse_cmp(
    _app: *mut c_void,
    na: c_int,
    pa: *const c_void,
    nb: c_int,
...
pub(super) unsafe extern "C" fn forward_cmp(
    _app: *mut c_void,
    na: c_int,
    pa: *const c_void,
    nb: c_int,
...
pub(super) unsafe fn texts(db: *mut Sqlite3, sql: &str) -> Vec<String> {
    let mut st: *mut Stmt = ptr::null_mut();
    let s = cs(sql);
    assert_eq!(
...
pub(super) static TRACE_LOG: std::sync::Mutex<Vec<String>> = std::sync::Mutex::new(Vec::new());
...
pub(super) fn trace_fnptr() -> *mut c_void {
...
pub(super) unsafe fn blob_open_ok(db: *mut Sqlite3, table: &str, col: &str, row: i64, rw: c_int) -> *mut c_void {
...
pub(super) unsafe fn blob_open_err(db: *mut Sqlite3, dbname: &str, table: &str, col: &str, row: i64, rw: c_int) -> (c_int, String) {
...
pub(super) unsafe fn errmsg(db: *mut Sqlite3) -> String {
    CStr::from_ptr(sqlite3_errmsg(db)).to_string_lossy().into_owned()
}

pub(super) unsafe fn blob_read_vec(b: *mut c_void, n: usize, off: c_int) -> (c_int, Vec<u8>) {
...
pub(super) fn auth_reset(verdict: Option<(c_int, c_int)>) {
    AUTH_LOG.lock().unwrap().clear();
    *AUTH_VERDICT.lock().unwrap() = verdict;
}

pub(super) fn auth_log() -> Vec<(c_int, String, String)> {
    AUTH_LOG.lock().unwrap().clone()
}

pub(super) unsafe fn set_auth(db: *mut Sqlite3) {
    assert_eq!(
...
pub(super) unsafe fn collect_pragma_col_names(db: *mut Sqlite3, table_arg: &str) -> Vec<String> {
...
pub(super) unsafe fn collect_col_text(db: *mut Sqlite3, sql: &str, n: i32) -> Vec<String> {
    let s = cs(sql);
    let mut st: *mut Stmt = ptr::null_mut();
    assert_eq!(
        sqlite3_prepare_v2(db, s.as_ptr(), -1, &mut st, ptr::null_mut()),
