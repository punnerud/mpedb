...
pub use admission::{agg_set_servable_by_index, parallel_fold_shape};
#[allow(unused_imports)] // only doc-links name it outside `admission`; the pre-split path is kept
pub use admission::agg_servable_by_index;
pub(crate) use derived::cte_working_table_def;
pub use derived::{DerivedPlan, RecursiveCtePlan};
pub use window::{Frame, FrameBound, FrameExclude, FrameMode, WindowFunc};
...
// 53: MULTI-ARGUMENT host aggregates (design/DESIGN-UDF.md stage 2, second half).
...
// 54: HOST COLLATING SEQUENCES in `ORDER BY` (design/DESIGN-UDF.md stage 3). The
