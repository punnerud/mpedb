...
/// `x IS [NOT] DISTINCT FROM y` — the SQL-standard spelling of the NULL-safe
/// comparison mpedb writes as `x IS [NOT] y`. The sense is INVERTED: "IS NOT
/// DISTINCT FROM" means EQUAL, so the two keywords flip the negation.
#[test]
fn is_distinct_from_is_the_standard_spelling_with_the_sense_flipped() {
    assert_eq!(
        expr("a IS DISTINCT FROM b"),
        Expr::IsDistinct(col("a"), col("b"), true)
    );
    assert_eq!(
        expr("a IS NOT DISTINCT FROM b"),
        Expr::IsDistinct(col("a"), col("b"), false)
    );
    // …and it agrees with the short form it is a synonym of.
    assert_eq!(expr("a IS NOT DISTINCT FROM b"), expr("a IS b"));
    assert_eq!(expr("a IS DISTINCT FROM b"), expr("a IS NOT b"));
    // NULL is an ordinary operand here, not the `IS NULL` shortcut.
    assert_eq!(
        expr("a IS DISTINCT FROM NULL"),
        Expr::IsDistinct(col("a"), Box::new(Expr::Lit(Value::Null)), true)
    );
}

/// A QUALIFIED right operand was the failing shape: without the `DISTINCT
/// FROM` arm the `FROM` fell out of the expression and was read as a second
/// FROM clause, so the error named a TABLE for what is a comparison.
#[test]
fn is_distinct_from_takes_a_qualified_right_operand() {
    let s = parse_statement("SELECT id FROM t WHERE t.col_a IS DISTINCT FROM t.col_b");
    assert!(s.is_ok(), "{:?}", s.err());
}

/// `GENERATED { ALWAYS | BY DEFAULT } AS IDENTITY` — PostgreSQL's identity
/// column, which shares its first word with sqlite's `GENERATED ALWAYS AS
/// (expr)` and means something else entirely.
///
/// Driven through `parse_ddl_dialect`, the door the wire server actually uses.
/// The first cut of this was tested through the expression helper instead, and
/// passed while the server still refused the statement: `BY` is a KEYWORD
/// token, so `eat_word("BY")` never matched, and only a test on the real path
/// could see it.
#[test]
fn identity_columns_parse_in_both_spellings() {
    for sql in [
        "CREATE TABLE t (id INTEGER GENERATED BY DEFAULT AS IDENTITY, v TEXT, PRIMARY KEY (id))",
        "CREATE TABLE t (id INTEGER GENERATED ALWAYS AS IDENTITY, v TEXT, PRIMARY KEY (id))",
        "CREATE TABLE t (id BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 5 INCREMENT BY 2))",
    ] {
        let r = crate::parse_ddl_dialect(sql, mpedb_types::Dialect::Postgres);
        assert!(r.is_ok(), "{sql}: {:?}", r.err());
    }
    // The sqlite generated-column form still parses, and is NOT an identity.
    let r = crate::parse_ddl_dialect(
...
/// The same discipline for `IS DISTINCT FROM`: through the statement parser
/// under the dialect the server runs.
#[test]
fn is_distinct_from_parses_on_the_postgres_path() {
    for sql in [
        ...
        for d in [mpedb_types::Dialect::Sqlite, mpedb_types::Dialect::Postgres] {
            let r = super::parse_statement_ctes_dialect(
                sql,
                &[],
                &[],
                &crate::binder::OpSet::default(),
                d,
            );
            assert!(r.is_ok(), "{sql} under {d:?}: {:?}", r.err());
        }
    }
}
