...
pub(super) fn plan_recursive_cte(
    ...
    let (a_stmt, a_ptypes, a_ctx, _a_list, a_out, a_subs) =
        plan_select(&rc.anchor, schema, n_params, catalog, mode, consts, None)?;
    ...
    let (r_stmt, r_ptypes, r_ctx, _r_list, r_out, r_subs) =
        plan_select(&rc.recursive, schema, n_params, catalog, mode, consts, Some(cte))?;
    ...
    let (o_stmt, o_ptypes, o_ctx, _o_list, o_out, o_subs) =
        plan_select(outer_ast, schema, n_params, catalog, mode, consts, Some(cte))?;
