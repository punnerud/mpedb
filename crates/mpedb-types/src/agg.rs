...
impl Accum {
    ...
    /// AFTER every value this one saw — the merge step of the partitioned
    /// parallel fold (design/DESIGN-PARALLEL-READ.md). Only the
    ...
    /// low bits are order-dependent), `group_concat` (order IS the answer),
    /// any DISTINCT accumulation (the dedup sets span partitions) — refuses:
    /// the parallel executor must never have produced them, so an arrival
    /// here is a bug surfacing, not a case to handle.
...
mod tests {
    ...
    /// of the concatenation — for every split point, which is the whole
    /// claim the parallel fold's merge stands on.
    #[test]
    ...
    /// first-strict-beat rule. Collation makes ties visible: under NOCASE,
    /// 'a' and 'A' are equal, and the spelling that survives must be the
    /// earlier segment's.
    #[test]
