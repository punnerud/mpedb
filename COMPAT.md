...
| GROUP BY | ✅ | columns, expressions (`GROUP BY a/100`), output ordinals (`GROUP BY 1`). **Bare (non-aggregated, non-grouped) columns are config-selectable** (`[compat] bare_group_by`, PLAN_FORMAT 30): `"sqlite"` (**the default**) accepts a bare column, `"postgres"` refuses it (`… must appear in GROUP BY …`) — and a `mirror import` from PostgreSQL is born `"postgres"`, so the strictness travels with the data's origin. In sqlite mode a bare column is accepted ONLY where its value is deterministic, so the answer still matches sqlite EXACTLY (never a guessed value — mpedb's core guarantee): **(1) never evaluated** — a `COALESCE(-24, col)` (or dead-`CASE`-branch) column that constant folding removes, the corpus-dominant case, so the result is the constant regardless of the row; **(2) min/max-determined** — a query with exactly one `min()`/`max()` and no other aggregate, where every bare column takes its value from the extremum's input row (a tie → the first such row; an all-NULL-argument group → that group's last row; an empty table → NULL — the exact sqlite 3.45 rule, differential-tested). A **genuinely-arbitrary** bare column — bare + a non-min/max aggregate, bare + BOTH min and max, or bare with no aggregate — is **REFUSED** in sqlite mode too (`… must appear in GROUP BY, be inside an aggregate, or be determined by a single min()/max()`) rather than return sqlite's arbitrary-row pick, which can differ across engines/versions. postgres mode refuses every bare column, matching PostgreSQL (verified vs PG 16) |
...
| Window functions | 🚧 | `<fn>() OVER ([PARTITION BY …] [ORDER BY … [ASC\|DESC]])` (design/DESIGN-WINDOW.md stage 1, PLAN_FORMAT 24). **Ranking**: `row_number()` (distinct 1..n, ties broken by scan/PK order), `rank()` (ties share, next skips: 1,1,3), `dense_rank()` (ties share, no gap: 1,1,2). **Aggregate OVER**: `sum`/`count(*)`/`count(x)`/`avg`/`min`/`max`/`total` with the DEFAULT frame only — with ORDER BY it is cumulative `RANGE UNBOUNDED PRECEDING → CURRENT ROW` (peers share one value: the running total through the end of their peer group, the RANGE-vs-ROWS distinction), without ORDER BY it is the whole partition. PARTITION BY groups NULLs together; window ORDER BY is NULLS FIRST for ASC (sqlite's default); the reused `Accum` keeps the aggregate NULL/overflow rules (integer `sum` overflow and `sum`/`avg` of text ERROR, as elsewhere). Multiple distinct windows per SELECT, windows in the SELECT list and the outer ORDER BY (junk-column path), and windows over a join all work. Computed AFTER WHERE, BEFORE the outer ORDER BY/LIMIT — cross-checked vs sqlite 3.45. **Refused (each a clean error, never a wrong answer)**: explicit frames (`ROWS`/`RANGE`/`GROUPS BETWEEN …`, stage 2), `lag`/`lead`/`first_value`/`last_value`/`nth_value`/`ntile`/`percent_rank`/`cume_dist` (stage 2), named `WINDOW w AS …` (stage 3), `FILTER`, `DISTINCT` inside a window aggregate (sqlite refuses it too), a window together with GROUP BY / an aggregate in the same SELECT, a window together with a correlated subquery, and a window anywhere but the SELECT list / ORDER BY (WHERE, HAVING, GROUP BY, a JOIN condition, an aggregate's argument, or nested inside another window) |
