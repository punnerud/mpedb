...
| Nested subqueries (subquery in a subquery) | 🚧 | uncorrelated nested (`x IN (SELECT … WHERE y IN (SELECT …))`, `EXISTS(… EXISTS(…))`, scalar-in-scalar, arbitrary depth) and a nested subquery correlated to its IMMEDIATE parent both work, cross-checked vs sqlite (#73 §3 stages 1–2). Correlation to a MIDDLE/outer scope (the scope-stack case) is still refused with a message — never a wrong answer |
| Window functions | ❌ | |
