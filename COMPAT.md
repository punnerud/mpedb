...
| CREATE TRIGGER / DROP TRIGGER | 🚧 | `AFTER INSERT … FOR EACH ROW [WHEN …] BEGIN <one INSERT/UPDATE/DELETE> END` fires (body runs on the same txn, `NEW.<col>` bound, depth-capped); stored as a sys-keyspace catalog record (no plan-format change). BEFORE, UPDATE/DELETE events, INSTEAD OF, FOR EACH STATEMENT, OLD, multi-statement bodies, and PySpell/`EXECUTE PROCEDURE` bodies are refused by name (later stages) — [DESIGN-TRIGGERS.md](DESIGN-TRIGGERS.md) |
| WITH (CTEs) | 🚧 | a non-recursive `WITH c AS (SELECT …) …` is a statement-scoped named source — flattened onto its base table at bind time via the derived-table keep-alias splice, so unqualified refs, qualified `c.col`, `FROM c AS x` (`x.col`), `SELECT *`, and joining a CTE with a base table all resolve (same simple projection/filter bodies as views). RECURSIVE, explicit column-lists `WITH c(x,y)`, a CTE in JOIN position, and aggregate/join/DISTINCT bodies are refused (never answered wrongly) — [DESIGN-CTE.md](DESIGN-CTE.md) |
