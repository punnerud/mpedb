...
unsupported edge is a named refusal (never a wrong answer). Deferred edges stay
named in the cell as deliberate non-goals or follow-ups — not silent holes. 🚧 is
reserved for surfaces still mid-implementation without a frozen refusal list.
...
| CREATE INDEX | ✅ | `CREATE [UNIQUE] INDEX [IF NOT EXISTS] n ON t (cols) [WHERE pred]` — built over existing rows, live + multi-process; ASC/DESC per column accepted (indexes are ascending, used for equality/prefix/range lookups). Or declare via config `unique`/`indexed`/`[[table.index]]`. The index name is not persisted (indexes are positional). **Partial indexes (P1):** a trailing `WHERE <literal pred>` is stored on `IndexDef.predicate` (canonical-bytes v10); the planner does not yet pick a partial for access (implication / P6), so SELECT stays correct via FullScan; non-deterministic host UDFs in the predicate are refused (sqlite 3.15+); `UNIQUE … WHERE` is refused until membership evaluation ships; parameterized predicates refuse until P6 |
...
| CREATE TRIGGER / DROP TRIGGER | ✅ | `{BEFORE\|AFTER} {INSERT\|UPDATE [OF cols]\|DELETE} … FOR EACH ROW [WHEN …] {BEGIN <stmts> END \| EXECUTE PROCEDURE p(args…)}` fires (body runs on the same txn, depth-capped; `NEW.<col>` for INSERT, `NEW`+`OLD` for UPDATE, `OLD` for DELETE; multi-statement bodies run in order; `UPDATE OF` fires on SET-target membership); stored as a sys-keyspace catalog record (no plan-format change). **`RAISE`**: `SELECT RAISE(ABORT,'msg') [WHERE …]` aborts with the message verbatim (C API: `SQLITE_CONSTRAINT_TRIGGER`), `SELECT RAISE(IGNORE) [WHERE …]` silently skips the row (per-row in a multi-row INSERT; in AFTER it abandons remaining trigger work only) — differential vs sqlite 3.45; `RAISE(FAIL\|ROLLBACK)` refused by name. **`EXECUTE PROCEDURE`** (PySpell, stage 5): dispatches to a stored procedure PINNED BY CONTENT HASH at CREATE (re-CREATE to rebind after redefining the proc), fired per row on the same txn — its SQL recurses in-transaction, a failing proc vetoes the write, budget-metered. NEW is read-only, and INSTEAD OF / FOR EACH STATEMENT / subqueries in the body are refused by name — [design/DESIGN-TRIGGERS.md](design/DESIGN-TRIGGERS.md) |
