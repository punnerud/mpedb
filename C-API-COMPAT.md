...
| 4 [3] | **44** | **Missing sqlite scalar functions**: `quote()` 40 statements (Django's `last_executed_query`), `strftime()` 8, `json()` (the feature probe → the 2 skips above). **JSONField's DDL now fails on ONE count rather than two**: Django emits `CHECK ((JSON_VALID("data") OR "data" IS NULL))`, and the `AND/OR requires boolean operands, got int64` half is gone with gap 5 — only `unknown function json_valid()` is left. | `mpedb-sql` builtins |
