...
- **M8 adversarial battery** (in progress): done — switch drill S2→S6→S2 ×N
  ...
  pull, model-checked (§10.4). Crash-safety cores (cursor guard / stale-replay,
  apply atomicity, never-wedge parking) covered by apply unit tests. *Remaining:*
  the multi-process SIGKILL `mirror-collide` + crash-waves-at-arrows harness
  (§10.4/§10.5, belongs with the CLI collide/crash infra); broader PG fidelity
  matrices (§10.8).
