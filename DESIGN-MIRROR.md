...
Status: **M1–M8 done**, plus type provenance (`mir/map`), pre-flight, import
adaptation, `export_pg`, and the §12 credential channel — the PostgreSQL path is
CLI-runnable and `sqlite3 → mpedb → PostgreSQL` works end to end (437 workspace
tests green, clippy clean; live-PG ignored tests pass serially: `cargo test -p
mpedb-mirror -- --ignored --test-threads=1`). M9 (v2 pgoutput slot) is next and
optional.

**Dialect rule (learned the hard way).** A recorded declared type only means
something in its own dialect, and the vocabularies collide: sqlite's `INTEGER`
is 64-bit where PG's `integer` is int4; sqlite's `REAL` is a double where PG's
`real` is float4. Reading a sqlite mirror with PG's rules made pre-flight reject
`5_000_000_000` out of a column sqlite stores it in natively — a false rejection
that then blocked the export. Anything consuming `mir/map` MUST branch on
`mir/cfg`'s `source_kind`: pre-flight applies width/precision checks only to
strict dialects (NOT NULL is enforced by both), and `export_pg` replays recorded
types only for a PG source, widening a sqlite one.
